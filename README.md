...
- **Импорт/экспорт** — данные можно выгружать и загружать в форматах
  **CSV** и **JSON** (клиенты, товары, заказы).
...
├── db.py              # слой хранения: Excel .xlsx, импорт/экспорт CSV/JSON
...
├── tests/             # unit-тесты (unittest): test_models.py, test_analysis.py
├── tools/
...
| `db.py` | Хранилище на базе рабочей книги Excel: листы `clients`, `products`, `orders`, `order_items`. CRUD, авто-сохранение, импорт/экспорт CSV и JSON, генератор демо-данных. |
...
При первом запуске хранилище автоматически заполняется демонстрационными
данными (30 клиентов, 20 товаров, ~137 заказов).
...
| Файлы и форматы | импорт/экспорт CSV и JSON; хранение между запусками в `.xlsx` |
...
| Unit-тесты | `tests/test_models.py`, `tests/test_analysis.py` (unittest, 55 тестов) |
